#!/bin/bash
#SBATCH --job-name=ltx_i2v_flowerbed_freezetime
#SBATCH --nodes=1
...
DATE_DIR=$(date +%Y-%m-%d)
LOG_DIR="/home/stud/deln/storage/user/projects/LTX-Video/slurm/logs/freezetime/flowerbed/${DATE_DIR}"
mkdir -p "$LOG_DIR"

# Move SLURM output files to the log directory
mv "slurm-${SLURM_JOB_ID}.out" "${LOG_DIR}/ltx_i2v_flowerbed-freezetime-${SLURM_JOB_ID}.out"
mv "slurm-${SLURM_JOB_ID}.err" "${LOG_DIR}/ltx_i2v_flowerbed-freezetime-${SLURM_JOB_ID}.err"
...
PROMPT="Camera circulating 360 degrees around the flowerbed. Time elapse. flowerbed is frozen in time. Freeze-time video, flowerbed showcases no movement."
TAG=$(echo "$PROMPT" | tr '[:upper:]' '[:lower:]' | tr -cd '[:alnum:]_')

RUNID=$(date +%Y%m%d_%H%M%S)
OUTDIR=$PROJECT_DIR/outputs/freezetime/flowerbed/${DATE_DIR}/$RUNID-$SEED
mkdir -p "$OUTDIR"
