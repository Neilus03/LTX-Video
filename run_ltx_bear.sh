...
DATE_DIR=$(date +%Y-%m-%d)
LOG_DIR="/home/stud/deln/storage/user/projects/LTX-Video/slurm/logs/bear/${DATE_DIR}"
mkdir -p "$LOG_DIR"

# Move SLURM output files to the log directory
mv "slurm-${SLURM_JOB_ID}.out" "${LOG_DIR}/ltx_i2v_bear-${SLURM_JOB_ID}.out"
mv "slurm-${SLURM_JOB_ID}.err" "${LOG_DIR}/ltx_i2v_bear-${SLURM_JOB_ID}.err"
...
OUTDIR=$PROJECT_DIR/outputs/bear/${DATE_DIR}/$RUNID-$SEED
mkdir -p "$OUTDIR"
